...
router.post("/", authMiddleware, async (req, res) => {
  try {
    const { name, phone, area, dailyCapacity, rating } = req.body;
...
// and lastAssignedDate are only ever touched by the /bookings/:id/assign route,
// so an admin edit here can't accidentally corrupt today's load tracking.
router.patch("/:id", authMiddleware, async (req, res) => {
  try {
    ...
    const partner = await DeliveryPartner.findByIdAndUpdate(
      req.params.id,
      { name, phone, area, dailyCapacity, rating },
